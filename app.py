import streamlit as st
import pandas as pd
import sqlite3
from datetime import datetime
import plotly.express as px
import re
import io

# --- CONFIGURAÇÕES INICIAIS ---
st.set_page_config(page_title="Gestor B3 - Trader Pro", layout="wide")

# Lista base caso o banco esteja vazio
BLUE_CHIPS = ["PETR4", "VALE3", "ITUB4", "BBDC4", "BBAS3", "ABEV3", "MGLU3"]

def init_db():
    conn = sqlite3.connect('investimentos.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS operacoes
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  data TEXT, ticket TEXT, tipo TEXT, 
                  quantidade INTEGER, valor REAL)''')
    conn.commit()
    conn.close()

def get_tickers_da_base():
    """Busca tickers já existentes no banco para sugerir ao usuário"""
    try:
        conn = sqlite3.connect('investimentos.db')
        df = pd.read_sql_query("SELECT DISTINCT ticket FROM operacoes", conn)
        conn.close()
        tickers_existentes = sorted(df['ticket'].tolist())
        return sorted(list(set(tickers_existentes + BLUE_CHIPS)))
    except:
        return BLUE_CHIPS

def validar_ticket(tkt):
    if not tkt or str(tkt).strip() == "":
        return False, "O campo Ticket não pode estar vazio."
    tkt_limpo = str(tkt).upper().strip()
    padrao = r'^[A-Z]{4}[0-9]{1,2}$'
    if not re.match(padrao, tkt_limpo):
        return False, f"Ticket '{tkt_limpo}' inválido. Use o padrão B3 (Ex: PETR4)."
    return True, ""

def salvar_operacao(data, ticket, tipo, qtd, valor):
    conn = sqlite3.connect('investimentos.db')
    c = conn.cursor()
    c.execute("INSERT INTO operacoes (data, ticket, tipo, quantidade, valor) VALUES (?,?,?,?,?)",
              (data, ticket.upper().strip(), tipo, qtd, valor))
    conn.commit()
    conn.close()

# --- FUNÇÃO EVOLUÍDA COM CORREÇÃO DE ERRO DE TIMESTAMP ---
def atualizar_banco_pelo_editor(df_editado, df_original):
    conn = sqlite3.connect('investimentos.db')
    try:
        # 1. Tratar exclusões: Identifica IDs que estavam no original mas não estão no editado
        # Usamos dropna() para ignorar linhas novas que ainda não tem ID
        ids_originais = set(df_original['id'].tolist())
        ids_atuais = set(df_editado['id'].dropna().tolist())
        
        for id_del in (ids_originais - ids_atuais):
            conn.execute("DELETE FROM operacoes WHERE id = ?", (int(id_del),))
        
        # 2. Tratar atualizações e inserções
        for _, row in df_editado.iterrows():
            ok, _ = validar_ticket(row['ticket'])
            if ok:
                # CORREÇÃO AQUI: Convertendo explicitamente a data para string formatada
                # O SQLite não aceita o objeto Timestamp do Pandas vindo do editor
                data_str = pd.to_datetime(row['data']).strftime('%Y-%m-%d')
                
                if pd.notna(row['id']):
                    # Atualizar registro existente
                    conn.execute('''UPDATE operacoes SET data=?, ticket=?, tipo=?, quantidade=?, valor=? 
                                    WHERE id=?''', (data_str, str(row['ticket']).upper().strip(), 
                                                   row['tipo'], int(row['quantidade']), float(row['valor']), int(row['id'])))
                else:
                    # Inserir novo registro caso o usuário tenha adicionado linha no editor
                    conn.execute('''INSERT INTO operacoes (data, ticket, tipo, quantidade, valor) 
                                    VALUES (?,?,?,?,?)''', (data_str, str(row['ticket']).upper().strip(), 
                                                           row['tipo'], int(row['quantidade']), float(row['valor'])))
        conn.commit()
    except Exception as e:
        st.error(f"Erro ao atualizar banco: {e}")
    finally:
        conn.close()

def calcular_tudo():
    conn = sqlite3.connect('investimentos.db')
    df = pd.read_sql_query("SELECT * FROM operacoes ORDER BY data ASC", conn)
    conn.close()
    if df.empty: return pd.DataFrame(), pd.DataFrame(), df

    df['data'] = pd.to_datetime(df['data'])
    vendas_realizadas = []
    controle = {} 

    for data in sorted(df['data'].dt.date.unique()):
        df_dia = df[df['data'].dt.date == data]
        for tkt in df_dia['ticket'].unique():
            if tkt not in controle: controle[tkt] = {'qtd': 0, 'pm': 0.0}
            ops = df_dia[df_dia['ticket'] == tkt]
            qc = ops[ops['tipo']=='Compra']['quantidade'].sum()
            qv = ops[ops['tipo']=='Venda']['quantidade'].sum()
            
            q_dt = min(qc, qv)
            if q_dt > 0:
                v_c, v_v = ops[ops['tipo']=='Compra']['valor'].mean(), ops[ops['tipo']=='Venda']['valor'].mean()
                vendas_realizadas.append({
                    'Data': data, 'Ticket': tkt, 'Qtd': q_dt, 'Tipo': 'Day Trade', 
                    'Resultado': (v_v - v_c)*q_dt, 'Volume Venda': q_dt*v_v
                })

            sqc, sqv = qc - q_dt, qv - q_dt
            if sqc > 0:
                v_compra_m = ops[ops['tipo']=='Compra']['valor'].mean()
                total_financeiro = (controle[tkt]['qtd'] * controle[tkt]['pm']) + (sqc * v_compra_m)
                controle[tkt]['qtd'] += sqc
                controle[tkt]['pm'] = total_financeiro / controle[tkt]['qtd']
            
            if sqv > 0:
                v_venda_m = ops[ops['tipo']=='Venda']['valor'].mean()
                lucro = (v_venda_m - controle[tkt]['pm']) * sqv
                vendas_realizadas.append({
                    'Data': data, 'Ticket': tkt, 'Qtd': sqv, 'Tipo': 'Swing Trade', 
                    'Resultado': lucro, 'Volume Venda': sqv * v_venda_m,
                    'PM_Venda': controle[tkt]['pm'], 'Preço_Venda': v_venda_m
                })
                controle[tkt]['qtd'] -= sqv

    df_res = pd.DataFrame(vendas_realizadas)
    if not df_res.empty:
        df_res['Data'] = pd.to_datetime(df_res['Data'])
        df_res['Mês/Ano'] = df_res['Data'].dt.to_period('M').astype(str)
    
    df_pos = pd.DataFrame([{'Ticket': t, 'Quantidade': d['qtd'], 'Preço Médio': d['pm'], 'Total': d['qtd']*d['pm']} for t, d in controle.items() if d['qtd'] > 0])
    return df_pos, df_res, df

@st.dialog("Sucesso")
def modal_sucesso(msg):
    st.success(msg)
    if st.button("OK"): st.rerun()

# --- INTERFACE ---
init_db()
if 'autenticado' not in st.session_state: 
    st.session_state['autenticado'] = False

if not st.session_state['autenticado']:
    st.title("🔐 Login")
    u, p = st.text_input("Usuário"), st.text_input("Senha", type="password")
    if st.button("Entrar"):
        if u == "admin" and p == "1234": 
            st.session_state['autenticado'] = True
            st.rerun()
else:
    pag = st.sidebar.radio("Menu", ["Registrar", "Posição", "Resultados & IR", "Gestão de Dados"])
    df_pos, df_res, df_raw = calcular_tudo()

    if pag == "Registrar":
        st.header("📝 Registrar Operação")
        tab_sugestao, tab_manual = st.tabs(["🔎 Rápido (Existentes na Base)", "⌨️ Novo Ativo (Manual)"])
        
        with tab_sugestao:
            with st.form("form_sugestao", clear_on_submit=True):
                c1, c2 = st.columns(2)
                lista_tickers = get_tickers_da_base()
                tkt_final = c1.selectbox("Selecione um ativo", [""] + lista_tickers)
                tipo = c1.selectbox("Tipo", ["Compra", "Venda"], key="tp1")
                qtd = c2.number_input("Quantidade", min_value=1, key="q1")
                valor = c2.number_input("Preço Unitário (R$)", min_value=0.0, format="%.2f", key="v1")
                data_op = st.date_input("Data", datetime.now(), key="d1")
                if st.form_submit_button("Registrar pela Sugestão"):
                    ok, msg = validar_ticket(tkt_final)
                    if ok:
                        salvar_operacao(data_op.strftime('%Y-%m-%d'), tkt_final, tipo, qtd, valor)
                        modal_sucesso(f"{tkt_final} registrado!")
                    else: st.error(msg)

        with tab_manual:
            with st.form("form_manual", clear_on_submit=True):
                c1, c2 = st.columns(2)
                tkt_manual = c1.text_input("Novo Ticket (Ex: KLBN11)")
                tipo = c1.selectbox("Tipo", ["Compra", "Venda"], key="tp2")
                qtd = c2.number_input("Quantidade", min_value=1, key="q2")
                valor = c2.number_input("Preço Unitário (R$)", min_value=0.0, format="%.2f", key="v2")
                data_op = st.date_input("Data", datetime.now(), key="d2")
                if st.form_submit_button("Salvar Novo Ativo"):
                    ok, msg = validar_ticket(tkt_manual)
                    if ok:
                        salvar_operacao(data_op.strftime('%Y-%m-%d'), tkt_manual, tipo, qtd, valor)
                        modal_sucesso(f"Ativo {tkt_manual.upper()} salvo!")
                    else: st.error(msg)

    elif pag == "Posição":
        st.header("🏢 Carteira Atual")
        if not df_pos.empty:
            st.dataframe(df_pos.style.format({'Preço Médio': 'R$ {:.2f}', 'Total': 'R$ {:.2f}'}), use_container_width=True, hide_index=True)
            st.plotly_chart(px.pie(df_pos, values='Total', names='Ticket', hole=0.4))
        else: st.info("Sua carteira está vazia.")

    elif pag == "Resultados & IR":
        st.header("📊 Performance e Detalhamento de Lucros")
        if not df_res.empty:
            res_m = df_res.groupby(['Mês/Ano', 'Tipo']).agg({'Resultado': 'sum', 'Volume Venda': 'sum'}).reset_index()
            for mes in sorted(res_m['Mês/Ano'].unique(), reverse=True):
                with st.expander(f"📅 Relatório Mensal: {mes}", expanded=True):
                    d_mes = res_m[res_m['Mês/Ano'] == mes]
                    v_st = d_mes[d_mes['Tipo']=='Swing Trade']['Volume Venda'].sum()
                    l_st = d_mes[d_mes['Tipo']=='Swing Trade']['Resultado'].sum()
                    l_dt = d_mes[d_mes['Tipo']=='Day Trade']['Resultado'].sum()
                    ir_st = (l_st * 0.15) if (v_st > 20000 and l_st > 0) else 0
                    ir_dt = max(0, l_dt * 0.20)
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Volume Vendas ST", f"R$ {v_st:,.2f}")
                    c2.metric("Lucro/Prejuízo Total", f"R$ {l_st + l_dt:,.2f}", 
                              delta=f"IR: R$ {ir_st + ir_dt:,.2f}", delta_color="inverse")
                    c3.markdown(f"**DARF sugerida:** :red[R$ {ir_st + ir_dt:,.2f}]")
                    st.write("---")
                    detalhe_vendas = df_res[df_res['Mês/Ano'] == mes].copy()
                    detalhe_vendas['Status'] = detalhe_vendas['Resultado'].apply(lambda x: "🟢 LUCRO" if x >= 0 else "🔴 PREJUÍZO")
                    st.dataframe(
                        detalhe_vendas[['Data', 'Ticket', 'Tipo', 'Qtd', 'Resultado', 'Status']].style.format({'Resultado': 'R$ {:.2f}', 'Data': lambda t: t.strftime('%d/%m/%Y')}),
                        use_container_width=True, hide_index=True
                    )
        else:
            st.info("Nenhuma venda realizada.")

    elif pag == "Gestão de Dados":
        st.header("⚙️ Backup e Manutenção")
        if not df_raw.empty:
            col1, col2 = st.columns(2)
            try:
                buf = io.BytesIO()
                with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
                    df_raw.to_excel(writer, index=False, sheet_name='Dados')
                col1.download_button("📥 Baixar Excel", buf.getvalue(), "backup_invest.xlsx")
            except: col1.warning("Erro ao gerar Excel.")
            col2.download_button("📥 Baixar CSV", df_raw.to_csv(index=False).encode('utf-8'), "backup_invest.csv")

        st.divider()
        st.subheader("Edição Direta do Banco")
        st.info("Para excluir: Selecione a linha e pressione 'Delete' no seu teclado. Para editar: Clique na célula.")
        df_edit = st.data_editor(df_raw, num_rows="dynamic", use_container_width=True)
        if st.button("Confirmar Alterações"):
            atualizar_banco_pelo_editor(df_edit, df_raw)
            st.rerun()

    if st.sidebar.button("Sair"): 
        st.session_state['autenticado'] = False
        st.rerun()